...
import type { Collection, DocumentFieldProps } from "@melony/core/config";
import { filterToQuery } from "@melony/core/filter";
import type { Adapter as MelonyAdapter } from "@melony/core/types";
import { dateReviver } from "@melony/core/utils";
import { type Db, type Document, type MongoClient, ObjectId } from "mongodb";
	...
	return {
		...
		getDocuments: async ({ collectionSlug, filter, sort, searchTerm }) => {
			...
			try {
				...
				const docFields: DocumentFieldProps[] = schema.filter(field => ["DOCUMENT"].includes(field.type || ""));
				...
				// query
				const docs = await db.collection(collectionSlug).aggregate(pipeline).toArray();
		...
		createDocument: async ({ collectionSlug, data, auth }) => {
			...
			const parsedBody = JSON.parse(serializedBody, dateReviver);
