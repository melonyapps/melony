...
export const serve = (
	config: Config,
): { GET: any; POST: any; PUT: any; DELETE: any } => {
	const { id, adapter = mongodbAdapter, collections, triggers = [] } = config;
		...
		triggers.map((trigger) => {
			if (trigger?.collectionSlug === changeEvent.collectionSlug) {
				if (trigger?.on?.includes(changeEvent.type)) {
					// execute
					trigger.func({ dbCrudAdapter, type: changeEvent.type }); // TODO: dbCrudAdapter or db? need to decide
				}
			}
		});
	});
